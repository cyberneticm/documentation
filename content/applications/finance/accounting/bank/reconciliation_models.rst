...
While default :ref:`matching rules <accounting/reconciliation/reconcile>` match and reconcile
transactions automatically based on a fixed set of rules, reconciliation models are customizable
rules that can also be used to automate the :doc:`bank reconciliation <reconciliation>` process.
These models are especially useful when dealing with recurring entries like bank fees and :doc:`cash
...
Each reconciliation model has its own :ref:`matching conditions <accounting/rec-models/conditions>`
that determine to which bank transactions it applies and its own :ref:`Counterpart Items
<accounting/rec-models/counterpart>` it will generate when it is used.
...
A reconciliation model's matching conditions determine to which transactions is applies. These
matching conditions are restrictive, meaning that leaving all the conditions blank will make the
reconciliation model available for all transactions whereas adding conditions will limit the
transactions for which the reconciliation model is available. The matching conditions include the
following fields:

- :guilabel:`Journals`: Limit the reconciliation model to only be available for transactions in
  certain bank or cash journals.
- :guilabel:`Partners`: Limit the reconciliation model to only be available for transactions
  involving certain partners.
- :guilabel:`Amount`: Limit the reconciliation model to only be available for transactions that are
  greater than or equal to a value, less than or equal to a value, or between two values.
- :guilabel:`Label`: Limit the reconciliation model to only be available for transactions that
  contain or do not contain a value, or use regex for even greater precision.
...
- :guilabel:`Taxes`: Select a tax, if any, to set on the journal item. This field is hidden behind
  the :icon:`oi-settings-adjust` :guilabel:`(settings adjust)` by default.
...
.. note::
   While neither the :guilabel:`Partner` nor :guilabel:`Account` fields are mandatory, at least one
   of the two must be set for the reconciliation model to work correctly. The reconciliation model
   can be used for :ref:`partner mapping <accounting/rec-models/partner>` if the
   :guilabel:`Counterpart Items` include a :guilabel:`Partner` but no :guilabel:`Account`.
...
:guilabel:`New`. The following reconciliation models exist across most fiscal localizations.
...
The :guilabel:`Cash Discount` reconciliation model generates a counterpart entry that moves the
